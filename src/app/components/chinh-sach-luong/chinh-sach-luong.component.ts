...
@Component({
  selector: 'app-chinh-sach-luong',
  templateUrl: './chinh-sach-luong.component.html',
  styleUrls: ['./chinh-sach-luong.component.scss']
})
export class ChinhSachLuongComponent implements OnInit, AfterViewChecked {
  ...
  delRow(event) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        const queryParams = queryString.stringify({ schemeId: event.rowData.schemeId });
        this.apiService.delSchemeInfo(queryParams)
        .pipe(takeUntil(this.unsubscribe$))
        .subscribe(results => {
  ...
  importFileExel() {
    this.router.navigate(['/cai-dat/chuc-danh/import-chuc-danh']);
  }
  ...
  getFilter() {
    // value === 1 ? '' : ''
    this.load();
    this.apiService.getFilter('/api/v1/schemeInfo/GetSchemeInfoFilter')
    .pipe(takeUntil(this.unsubscribe$))
    .subscribe(results => {
      if (results.status === 'success') {
