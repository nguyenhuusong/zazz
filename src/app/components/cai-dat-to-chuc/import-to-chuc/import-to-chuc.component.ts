...
@Component({
  selector: 'app-import-to-chuc',
  templateUrl: './import-to-chuc.component.html',
  styleUrls: ['./import-to-chuc.component.scss']
})
export class ImportToChucComponent implements OnInit {
  items: any = [];
  isFullScreen = false;
  heightGrid = 0
  isShowUpload = true;
  ...
  onSelectFile(event) {
    if (event.currentFiles.length > 0) {
      ...
      this.apiService.setOrganizeImport(fomrData)
  ...
  checkData(accept = false) {
    ...
    this.apiService.setOrganizeAccept(params)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        this.dataSet(results)
      })
  ...
  getTemfileImport() {
    this.apiService.exportReportLocalhost('assets/tpl-import-file/file_mau_import_danh_muc_to_chuc.xlsx').subscribe((data: any) => {
  ...
  createImageFromBlob(image: Blob) {
    ...
    anchor.download = "file_mau_import_danh_muc_to_chuc.xlsx";
    anchor.href = url;
    anchor.click();
  }
