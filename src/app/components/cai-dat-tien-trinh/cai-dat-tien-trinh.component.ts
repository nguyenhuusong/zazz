...
@Component({
  selector: 'app-cai-dat-tien-trinh',
  templateUrl: './cai-dat-tien-trinh.component.html',
  styleUrls: ['./cai-dat-tien-trinh.component.scss']
})
export class CaiDatTienTrinhComponent implements OnInit, AfterViewChecked {
  ...
  load() {
    ...
    this.apiService.getFlowTypePage(queryParams)
  ...
  delRow(event) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.apiService.delFlowType(queryParams)
        .pipe(takeUntil(this.unsubscribe$))
        .subscribe(results => {
  ...
  geFilter() {
    this.apiService.getFilter('/api/v2/workflowtype/GetflowTypeFilter')
    .pipe(takeUntil(this.unsubscribe$))
    .subscribe(results => {
      if(results.status === 'success') {
