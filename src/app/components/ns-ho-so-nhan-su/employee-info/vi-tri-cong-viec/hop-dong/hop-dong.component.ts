...
export class HopDongComponent implements OnInit {
  ...
  getProcessMetaPage() {
    this.spinner.show();
    this.columnDefs = [];
    const queryParams = queryString.stringify({ processId: this.processId, offSet: 0, pageSize: 10000 });
    this.apiService.getProcessMetaPage(queryParams)
  ...
  getContractRecord() {
    ...
    const queryParams = queryString.stringify({ processId: this.processId});
  ...
  handleUpload(event) {
    ...
    formData.append('sourceId', this.processId);
